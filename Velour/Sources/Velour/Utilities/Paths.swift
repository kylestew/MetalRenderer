...
public func getPipelinesSatinPath() -> String? {
    if let satinPath = getPipelinesPath("Velour") {
        return satinPath
    }
    return nil
}
...
public func getPipelinesVelourUrl() -> URL? {
    return getPipelinesUrl("Velour")
}

public func getPipelinesVelourUrl(_ path: String) -> URL? {
    return getPipelinesVelourUrl()?.appendingPathComponent(path)
}
